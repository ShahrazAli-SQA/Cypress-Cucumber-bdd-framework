...
import { TESTDATA, adminPage } from "../constant/admin";
...
Given("login admin portal and goto transactions page", () => {
  adminPage.login();
...
When("user click on {string} tab", (tab) => {
  cy.get(transactionPage.transactionTabs).contains(tab).click();
  cy.get(transactionPage.transactionTabs)
    .contains(tab)
    .parent()
    .verifyBgcolor("#4c6fff");
});
...
Then("user should create new investor", () => {
  ...
  investorEmail = adminPage.generateEmail();
  investorPhone = adminPage.generatePhoneNo();
  investorCnic = adminPage.generateCnic();
  adminPage
    .createInvestor(investorName, investorEmail, investorPhone, investorCnic)
    .then((investor) => {
...
When("user click on {string} transaction", (type) => {
  cy.get(transactionPage.selectTransactionType).contains(type).click();
});

And("add transaction required information", () => {
  cy.get(transactionPage.transactionFormDropdown).first().click();
  cy.get(transactionPage.dropdownOption).first().click();
  cy.get(transactionPage.transactionFormDropdown).eq(1).click();
  cy.get(transactionPage.dropdownOption).eq(1).click();
  cy.get(transactionPage.transactionFormDropdown).eq(2).click();
  cy.get(transactionPage.dropdownOption).clickRandomOption();
  ...
  cy.get(transactionPage.investorsList).first().click();
  cy.get(transactionPage.peerAreaInput).clearAndType(
    Math.floor(Math.random() * 50) + 1,
  );
  cy.get(transactionPage.transactionFormDropdown).last().click({ force: true });
...
And("click on proceed button", () => {
  cy.xpath(transactionPage.proceedBtn).should("be.enabled").click();
});

And("click on proceed next button", () => {
  cy.xpath(transactionPage.proceedNextBtn).first().should("be.enabled");
  cy.xpath(transactionPage.proceedNextBtn).first().click();
});

Then("user should be able to add project information", () => {
  cy.get(transactionPage.transactionFormDropdown)
    .invoke("css", "opacity", "1")
    .should("be.visible");
});

Then("user should be able to see transaction summary", () => {
  adminPage.verifyPageTitle("Transaction Summary");
...
Then("user should be on otp modal", () => {
  adminPage.verifyPageTitle("OTP Verification");
});
...
Then("{string} transaction should created successfully", (type) => {
  if (type === "partner plan") {
    ...
    cy.xpath(transactionPage.backToTransactionBtn).click();
...
And("click proceed next button", () => {
  ...
  cy.xpath(transactionPage.proceedNextBtn).last().click();
});
...
Then("edit transaction page should be visible", () => {
  adminPage.verifyPageTitle("Edit Transaction");
});
...
Then("update transaction status screen should be visible", () => {
  ...
  adminPage.verifyPageTitle("Update status");
});
...
And("select {string} filter option", (days) => {
  cy.get(transactionPage.daysFilterMenu).find("button").contains(days).click();
});

Then("{string} should be selected", (days) => {
  cy.get(transactionPage.daysFilterBtn)
    .invoke("text")
    .then((text) => {
      text = text.trim();
      expect(text).to.equal(days);
    });
});
...
When("user search transaction id", () => {
  ...
  cy.xpath(transactionPage.transactioIdIndex)
    ...
    .then(() => {
      if (transID) {
        cy.log("Found element with 5 characters: " + transID);
...
Then("transaction details should be visible", () => {
  adminPage.verifyPageTitle("Transaction Details");
});
...
Then("update document page should be visible", () => {
  adminPage.verifyPageTitle("Update Documents");
});
