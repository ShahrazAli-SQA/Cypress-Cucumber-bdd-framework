Feature: Admin Portal | Transactions Module

    Checking Transactions page functionality

    Background: login admin portal and goto transactions page
        Given login admin portal and goto transactions page

    Scenario Outline: user should be able to explore <tab> transactions
    ...
    Scenario Outline: user should be able to filter transaction for <Days>
        When user click on days filter button
        And select "<Days>" filter option
        Then "<Days>" should be selected
        Examples:
            | Days |
            |  Last 7 Days   |
            |  Last 15 Days  |
